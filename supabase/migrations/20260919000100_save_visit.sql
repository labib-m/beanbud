...
-- Bean Bud — save_visit()
--
-- Saving a visit touches up to four tables (cafe, visit, drinks, note).
-- A browser can't wrap several requests in one transaction, so a dropped
-- connection could leave a visit without its drinks. This function does
-- all of it as ONE unit: either everything is saved or nothing is.
--
-- It is "security invoker" (the default): it runs with the CALLER's
-- rights, so every row-level security policy still applies exactly as if
-- the app had made each write itself. It cannot do anything the caller
-- couldn't already do.
-- =====================================================================
begin;
